...
```{r}
# c é a função para concatenar, precisa para a matriz
# nrow e ncol vai definir a dimensão da matriz
# byrow vai definir a ordem que vai ser lida seus dados, por coluna(false) ou por linha(true)
M1 <- matrix(c(1:9), nrow = 3, ncol = 3, byrow = FALSE)
M1
...
M6 <- matrix(c(1:4), nrow=2, ncol=2, byrow=TRUE)
M6

M7 <- matrix(c(-1, 2, 3, 5, 10, -2), nrow=2, ncol=3, byrow=TRUE)
M7

M8 <- matrix(c(1, 3, 0, 2, 5, 1, 2, 1, 3), nrow=3, ncol=3, byrow=TRUE)
M8

M9 <- matrix(c(3, 0, -2, 1, 4, -1), nrow=3, ncol=2)
M9
...
M10 <- M3 + M7
M10
# Podemos somar e subtrair matrizes somente se possuierem a mesma dimensão
